...
const Testimonials = () => {
  ...
  return <section className="py-24 px-4 relative overflow-hidden">
      ...
      <div className="absolute bottom-0 left-0 w-80 h-80 bg-secondary/5 rounded-full blur-3xl translate-y-1/2 -translate-x-1/3" style={{
      transform: `translate(-30%, 50%) translateY(-${scrollY * 0.05}px)`
    }}></div>
      ...
      <div className="container mx-auto relative">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="relative h-80">
            {testimonials.map((testimonial, index) => <motion.div key={testimonial.id} className="absolute top-0 left-0 w-full" initial={{
            opacity: 0,
            y: 20
          }} animate={{
          ...
          <div className="flex justify-center space-x-2 mt-8">
            {testimonials.map((_, index) => <button key={index} onClick={() => setCurrentIndex(index)} className={`w-3 h-3 rounded-full transition-colors ${index === currentIndex ? 'bg-primary' : 'bg-muted'}`} aria-label={`Go to testimonial ${index + 1}`} />)}
          </div>
        </div>
      </div>
    </section>;
